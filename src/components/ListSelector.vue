<template>
  <div>
    <q-select
      ...
      :label="
        type == 'noga'
          ? $t('selector.dropdowns.industry')
          : $t('selector.dropdowns.occupation')
      "
      v-model="model"
      use-input
      @filter="filterFn"
    ...
    <q-list>
      <q-item v-for="s in selection" :key="s.byvalue">
  ...
  computed: {
    availableOptions: function () {
      return this.activeOptions.filter(
        (o) => this.selection.map((s) => s.byvalue.value).indexOf(o.value) < 0,
      );
    },
  },
  methods: {
    ...
    filterFn: function (val, update) {
      update(() => {
        this.activeOptions = this.options.filter(
          (o) => o.label.toLowerCase().indexOf(val.toLowerCase()) >= 0,
        );
      });
    },
  },
